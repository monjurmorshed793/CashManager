...
export class ExpanseComponent implements OnInit, OnDestroy {
  expanses?: IExpanse[];
  ...
  trackId(index: number, item: IExpanse): number {
  ...
  registerChangeInExpanses(): void {
    this.eventSubscriber = this.eventManager.subscribe('expanseListModification', () => this.loadPage());
  }

  delete(expanse: IExpanse): void {
    const modalRef = this.modalService.open(ExpanseDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.expanse = expanse;
  }
